...
class KiralamaService(BaseService):
    ...
    def get_next_form_no():
        """Sıradaki kiralama form numarasını (PF-YIL/SIRA) otomatik üretir."""
        max_id = db.session.query(func.max(Kiralama.id)).scalar() or 0
        year = datetime.now().year
        return f"PF-{year}/{max_id + 1:04d}"
    ...
    def update_kiralama_with_relations(cls, kiralama_id, kiralama_data, kalemler_data, actor_id=None):
        ...
        try:
            ...
            Nakliye.query.filter_by(kiralama_id=kiralama.id).delete(synchronize_session=False)
    ...
    def _create_nakliye_ve_cari(kiralama, kalem, makine_adi, bas_tarihi):
        ...
        yeni_sefer = Nakliye(
            ...
            guzergah=f"Kiralama Sevkiyatı ({makine_adi})", tutar=to_decimal(kalem.nakliye_satis_fiyat),
            kdv_orani=kiralama.kdv_orani or 20, aciklama=f"Sistem: {kiralama.kiralama_form_no}"
        )
